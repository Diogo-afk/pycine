import requests
api_key = "021db3c3f73aae753776b02b95267647"
...
def get_artista_name(name):
    """ Dado o nome do artista, retorna informações sobre ele """
    endpoint = "/search/person"
    params = f"?query={name.replace(' ', '+')}"
    ...
    if results:
        artist = results[0]  # Assume o primeiro resultado como o artista desejado
        print(f"Nome: {artist['name']}")
        print(f"ID: {artist['id']}")
    else:
        print(f"Nenhum artista encontrado com o nome '{name}'")
